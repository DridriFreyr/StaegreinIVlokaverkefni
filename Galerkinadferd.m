%% FAllið Galerkinadferd er gert til þess að fá nálgun með aðferð Galerkins á þúfugrunnföllum.
% Inntök fyrir fallið eru föllin p(x),q(x),f(x) ásamt  jaðargildisstuðlum: alpha, beta og gamma.
% Einnig býður fallið upp á að notandi setji inn punktuppsprettur í punkti r af stærð Q.
% Fallið skilar svo vigri c með stuðlum c_i sem er nálgun á lausn u(x_i).
% ATH: scriptið er að mestu skrifað eftir uppskrift frá Ragnari Sigurðssyni úr fyrirlestranótum hans frá janúar 2015.
...
% Skilgreinum punkta m sem eru staðsetningar á millipunktum.
m=(x(2:N)+x(1:N-1))/2; 
pm=p(m); 
qm=q(m);
fx=f(x);
% Byrjum á að búa til núllfylki og núll-vigur fyrir A og b,
A=zeros(N,3); 
...
% Fyllum svo inn í fyrstu línurnar, eftir því hvað betan er.
if beta(1) == 0
    A(1,2) = alpha(1);
    A(1,3) = 0;
    b(1) = gamma(1);
